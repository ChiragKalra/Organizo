...
class CategoryActivity : SimpleActivity(), DirectoryOperationsListener {
    ...
    private fun getDirectories() {
        ...
        ensureBackgroundThread {
            val it = categoryDao.getAllExceptOther().map { it.toDirectory() }
            gotDirectories(arrayListOf<Directory>().apply {
                addAll(it)
            })
        }
    }
    ...
    private fun setupAdapter(dirs: ArrayList<Directory>, textToSearch: String = "", forceRecreate: Boolean = false) {
        ...
        if (currAdapter == null || forceRecreate) {
            ...
            DirectoryAdapter(
                ...
                it as Directory
                Intent(this, MediaCategoryActivity::class.java).apply {
                    putExtra(SKIP_AUTHENTICATION, true)
                    putExtra(DIRECTORY, it.id?.toInt())
                    handleMediaIntent(this)
                }
            }.apply {
